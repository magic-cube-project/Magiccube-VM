/*
 * Description: 
 *     History: lemon@mofang.com, 2018/08/16, create
 */
...
// reply_error_invalid_argument_message
static int reply_error_invalid_argument_message(nw_ses *ses, rpc_pkg *pkg, char *message)
{
    return reply_error(ses, pkg, 1, message);
}
...
static int on_cmd_balance_query(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    if (request_size == 0){
        log_error("the params is null");
        return reply_error_invalid_argument_message(ses, pkg,"the params is null");
    }

    if (!json_is_integer(json_array_get(params, 0)))
    {
        log_error("user_is is not intrger");
        return reply_error_invalid_argument_message(ses, pkg,"user_id is not intrger");
    }
    uint32_t user_id = json_integer_value(json_array_get(params, 0));
    if (user_id == 0){
        log_error("user_id is 0");
        return reply_error_invalid_argument_message(ses, pkg,"user_id is 0");
    }
    // get balance num
    json_t *result = json_object();
    if (request_size == 1) {
...
static int on_cmd_balance_update(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    // user_id
    if (!json_is_integer(json_array_get(params, 0)))
        return reply_error_invalid_argument_message(ses, pkg,"user_id is invalid");
    uint32_t user_id = json_integer_value(json_array_get(params, 0));

    // asset
    if (!json_is_string(json_array_get(params, 1)))
        return reply_error_invalid_argument_message(ses, pkg,"asset is invalid");
    const char *asset = json_string_value(json_array_get(params, 1));
    int prec = asset_prec_show(asset);
    if (prec < 0)
        return reply_error_invalid_argument_message(ses, pkg,"asset_prec is invalid");

    // business
    if (!json_is_string(json_array_get(params, 2)))
        return reply_error_invalid_argument_message(ses, pkg,"business is invalid");
    const char *business = json_string_value(json_array_get(params, 2));

    // business_id
    if (!json_is_integer(json_array_get(params, 3)))
        return reply_error_invalid_argument_message(ses, pkg,"business_id is invalid");
    uint64_t business_id = json_integer_value(json_array_get(params, 3));

    // change
    if (!json_is_string(json_array_get(params, 4)))
        return reply_error_invalid_argument_message(ses, pkg,"change is invalid");
    mpd_t *change = decimal(json_string_value(json_array_get(params, 4)), prec);
    if (change == NULL)
        return reply_error_invalid_argument_message(ses, pkg,"change is null");

    // detail
    json_t *detail = json_array_get(params, 5);
    if (!json_is_object(detail)) {
        mpd_del(change);
        return reply_error_invalid_argument_message(ses, pkg,"detail is invalid");
    }
...
static int on_cmd_order_put_limit(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    if (json_array_size(params) != 8)
        return reply_error_invalid_argument_message(ses, pkg,"json is null");
    ...
    // market
    if (!json_is_string(json_array_get(params, 1)))
        return reply_error_invalid_argument_message(ses, pkg,"market is invalid");
    const char *market_name = json_string_value(json_array_get(params, 1));
    market_t *market = get_market(market_name);
    if (market == NULL)
        return reply_error_invalid_argument_message(ses, pkg,"market is null");

    // side
    if (!json_is_integer(json_array_get(params, 2)))
        return reply_error_invalid_argument_message(ses, pkg,"side is invalid");
    uint32_t side = json_integer_value(json_array_get(params, 2));
    if (side != MARKET_ORDER_SIDE_ASK && side != MARKET_ORDER_SIDE_BID)
        return reply_error_invalid_argument_message(ses, pkg,"side != MARKET_ORDER_SIDE_ASK && side != MARKET_ORDER_SIDE_BID");
