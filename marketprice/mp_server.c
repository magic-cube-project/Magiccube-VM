/*
 * Description: 
 *     History: lemon@mofang.com, 2018/09/18, create
 */
...
static int reply_error_invalid_argument_msg(nw_ses *ses, rpc_pkg *pkg,char *msg)
{
    return reply_error(ses, pkg, 1, msg);
}
...
static int reply_result(nw_ses *ses, rpc_pkg *pkg, json_t *result)
{
    ...
    json_object_set_new(reply, "id", json_integer(pkg->req_id));
    // json_object_set_new(replay,"",json_string(pkg->req_id));
...
static int on_cmd_market_kline(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    if (json_array_size(params) != 4)
        return reply_error_invalid_argument_msg(ses, pkg,"params is not four");

    const char *market = json_string_value(json_array_get(params, 0));
    if (!market)
        return reply_error_invalid_argument_msg(ses, pkg,"market is not invalid");
    if (!market_exist(market))
        return reply_error_invalid_argument_msg(ses, pkg,"market is not exist");

    time_t start = json_integer_value(json_array_get(params, 1));
    if (start <= 0)
        return reply_error_invalid_argument_msg(ses, pkg,"start is <=0");
    ...
    if (end <= 0 || start > end)
        return reply_error_invalid_argument_msg(ses, pkg,"end is invalid");

    int interval = json_integer_value(json_array_get(params, 3));

    log_debug("interval %d",interval);
    if (interval <= 0)
        return reply_error_invalid_argument_msg(ses, pkg,"interval is not <= 0");
...
static int on_cmd_market_last(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    const char *market = json_string_value(json_array_get(params, 0));
    if (!market){
        log_error("the market is null");
        return reply_error_invalid_argument(ses, pkg);
    }
    if (!market_exist(market))
    {   log_error("the market < %s> is not exist",market);
        return reply_error_invalid_argument(ses, pkg);
    }
